...
def coin_v2(
    ...
    def compute_loss_q(data):
        ...
        cur_q_a = torch.gather(cur_q, dim=1, index=a.long()).squeeze(-1)
        ...
        with torch.no_grad():
            ...
            td_error = (targ_q_coin_a - cur_q_a).mean().item()
            logger.store(TDErr=td_error)
    ...
    for t in range(total_steps):
        ...
        # if np.random.rand(1).item() < 1.0 - gamma:
        #     bonus *= 0.99
        #     replay_buffer.update_coin_rewards(bonus)
        #     cum_bonus += bonus
        bonus *= bonus_decay
        logger.store(Bonus=bonus)
        cum_bonus += bonus
