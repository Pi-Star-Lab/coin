...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, size):
        ...
        self.is_div_act_buf = np.zeros(size, dtype=np.float32)
        self.ptr, self.size, self.max_size = 0, 0, size

    def store(self, obs, act, rew, coin_rew, next_obs, next_act, done, ret, is_div_act):
        """
        ...
        self.is_div_act_buf[self.ptr] = is_div_act
    ...
    def sample_batch(self, batch_size=32):
        ...
        batch = dict(
            ...
            is_div_act=self.is_div_act_buf[idxs],
        )
...
def coinrdiv(
    ...
    """
    Regret-bounded Continual Optimistic Initialization (COINR) for discrete action spaces.
    Args:
    ...
    def compute_loss_q(data):
        ...
        o, a, r, cr, o2, a2, d, ret, is_div_act = (
            data["obs"],
            data["act"],
            data["rew"],
            data["coin_rew"],
        ...
        with torch.no_grad():
            ...
            if base_q_net is not None:
                ...
                if isinstance(base_q_net, core.COINQFunction):
                    base_max_q, _ = base_q_net.q_coin(o).max(dim=1)
                elif isinstance(base_q_net, dqn_core.DQNQFunction):
                    base_max_q, _ = base_q_net.q(o).max(dim=1)
                ...
                # eta = (base_max_q - ret) / regret_bound
                eta = (base_perf - ret) / regret_bound
                # ==== Correct eta ======
                # eta = (base_perf - ret) / (regret_bound - (base_perf - ret))

                # The Q-value we want to achieve
                targ_regret = delta

                # If the (avg.) return is greater than the base best action
                is_better_act = torch.gt(ret, base_max_q).float().squeeze(-1)
                ...
                backup_coin_a = (
                    (1 - is_div_act) * targ_q_coin_a
                    + is_div_act * is_better_act * targ_q_coin_a
                    + is_div_act
                    * (1 - is_better_act)
                ...
                mse_weights_a = (
                    (1 - is_div_act) * torch.ones_like(targ_q_coin_a)
                    + is_div_act * is_better_act * torch.ones_like(targ_q_coin_a)
                    + is_div_act
                    * (1 - is_better_act)
                    * eta
    ...
    for t in range(total_steps):
        ...
        if isinstance(base_q_net, core.COINQFunction):
            base_a = base_q_net.q_coin(torch.as_tensor(o, dtype=torch.float32)).argmax(
                dim=-1
            )
        elif isinstance(base_q_net, dqn_core.DQNQFunction):
            base_a = base_q_net.q(torch.as_tensor(o, dtype=torch.float32)).argmax(
                dim=-1
            )
        if is_div_a == 1:
            is_div_a = None
        if is_div_a == 0 and a.squeeze(-1) != base_a.float():
            is_div_a = 1
        ...
        rollout.append((o, a, r, cr, o2, a2, d, 0 if is_div_a is None else is_div_a))
        ...
        if d or (ep_len == max_ep_len):
            ...
            for i in range(len(rollout) - 2, -1, -1):
                ret[i] = rollout[i][2] + gamma * ret[i + 1]
            # Add all experiences in the episode to replay buffer
            for i, transition in enumerate(rollout):
                o, a, r, cr, o2, a2, d, is_div_a = transition
                replay_buffer.store(o, a, r, cr, o2, a2, d, ret[i], is_div_a)
